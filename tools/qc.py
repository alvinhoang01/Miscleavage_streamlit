...
def qc_one_trypsinp(path, output_dir, sqlite_path, enz):
    ...
    df_nodup = df_na.drop_duplicates().copy()
    ...
    df_nodup = pd.DataFrame(rows_list)
    ...
    del pep_map  # Free memory from the cached mapping
    gc.collect()
    ...
    df_mc2 = calc_quant_for_fragment_pep(df_mc, pep_quant_map, pep_quant_map, sample)
    ...
    for index, row in df_mc2.iterrows():
        ...
        rows_list.append(row)
    # Concatenate rows from df_mc (as per original code)
    df_mc2 = pd.concat([pd.DataFrame([row]) for row in tqdm(df_mc.iterrows())], ignore_index=True)
